<template>
    ...
    >
        ...
        >
            ...
            >
                ...
                <div class="hidden md:flex items-center gap-8">
                    <NuxtLink to="/" class="flex items-center gap-1 text-sm font-bold uppercase tracking-wide hover:text-primary transition-colors">
                        <span class="font-mono text-primary text-[10px] -mt-3">00</span>Home
                    </NuxtLink>
                    <NuxtLink v-if="isGym" to="/gym" id="nav-log" class="flex items-center gap-1 text-sm font-bold uppercase tracking-wide hover:text-primary transition-colors">
                        <span class="font-mono text-primary text-[10px] -mt-3">01</span>Gym Log
                    </NuxtLink>
                    <NuxtLink v-else to="/calist" id="nav-log" class="flex items-center gap-1 text-sm font-bold uppercase tracking-wide hover:text-primary transition-colors">
                        <span class="font-mono text-primary text-[10px] -mt-3">01</span>Calist Log
                    </NuxtLink>
                    <NuxtLink to="/weight" id="nav-weight" class="flex items-center gap-1 text-sm font-bold uppercase tracking-wide hover:text-primary transition-colors">
                        <span class="font-mono text-primary text-[10px] -mt-3">02</span>Weigh In
                    </NuxtLink>
                    <NuxtLink to="/coach" id="nav-coach" class="flex items-center gap-1 text-sm font-bold uppercase tracking-wide hover:text-primary transition-colors text-primary">
                        <span class="font-mono text-primary text-[10px] -mt-3">03</span>AI Coach
                    </NuxtLink>
                </div>
                ...
                <div class="flex items-center gap-4">
                    <div class="hidden md:flex items-center gap-3">
                        ...
                        <div v-if="isAuthenticated" class="relative">
                            <button @click="isUserMenuOpen = !isUserMenuOpen" class="flex items-center gap-2 hover:bg-black/5 px-2 py-1.5 rounded transition-colors focus:outline-none">
                            ...
                            <transition name="fade">
                                <div v-if="isUserMenuOpen" class="absolute right-0 mt-3 w-48 bg-white border-2 border-foreground-primary shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] z-50 flex flex-col">
                                    <NuxtLink to="/profile" @click="isUserMenuOpen = false" class="px-4 py-3 text-xs font-bold uppercase tracking-wider hover:bg-[#fcfbf7] transition-colors text-left flex items-center gap-2 border-b border-separator">
                                        <User class="w-4 h-4" />Profile Settings
                                    </NuxtLink>
                                    <button @click="handleLogoutClick" class="px-4 py-3 text-xs font-bold uppercase tracking-wider text-red-600 hover:bg-red-50 transition-colors text-left flex items-center gap-2">
                        ...
                        <NuxtLink v-else to="/login" class="px-4 py-2 border border-black rounded font-bold text-xs uppercase hover:bg-black hover:text-white transition-colors">
                            Login
                        </NuxtLink>
                    </div>

                    <button @click="isMenuOpen = !isMenuOpen" class="md:hidden p-2 hover:bg-black/5 rounded-lg transition-colors focus:outline-none">
            ...
            <transition name="menu">
                <div v-if="isMenuOpen" class="md:hidden absolute top-20 left-0 w-full bg-background border-b border-separator h-[calc(100vh-5rem)] overflow-y-auto z-40">
                    <div class="inner py-8 flex flex-col gap-6 px-4">
                        ...
                        <NuxtLink v-if="isGym" to="/gym" id="mob-log" @click="isMenuOpen = false" class="text-4xl font-black uppercase hover:text-primary transition-colors block">
                            <span class="text-primary text-sm font-mono mb-1 block">01</span>GYM LOG
                        </NuxtLink>
                        <NuxtLink v-else to="/calist" id="mob-log" @click="isMenuOpen = false" class="text-4xl font-black uppercase hover:text-primary transition-colors block">
                            <span class="text-primary text-sm font-mono mb-1 block">01</span>CALIST LOG
                        </NuxtLink>
                        <NuxtLink to="/weight" id="mob-weight" @click="isMenuOpen = false" class="text-4xl font-black uppercase hover:text-primary transition-colors block">
        ...
        <footer class="py-12 text-center text-foreground-text border-t border-separator bg-background">
            <div class="inner flex flex-col items-center gap-4">
                <p class="font-handwriting text-2xl">Consistency beats intensity.</p>
                <p class="text-xs font-mono opacity-60 uppercase tracking-widest">© 2026 BodyLog</p>
            </div>
        </footer>

        <transition name="fade">
            <div v-if="showModeModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
                <div class="w-full max-w-lg bg-white border-2 border-foreground-primary shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative animate-bounce-in">
                    <div class="p-8 border-b border-separator">
                        ...
                        <h3 class="text-4xl font-black uppercase text-foreground-primary">Pilih Mode Latihan</h3>
                    </div>

                    <div class="grid grid-cols-2 divide-x divide-separator">
                        <button @click="selectMode('gym')" class="p-8 text-left hover:bg-[#fcfbf7] transition-colors group flex flex-col gap-4">
                            <Dumbbell class="w-10 h-10 text-primary group-hover:-rotate-12 transition-transform" :stroke-width="1.5" />
                            <div>
                                <h4 class="text-2xl font-black uppercase text-foreground-primary group-hover:text-primary transition-colors">GYM</h4>
                                <p class="font-mono text-xs text-foreground-text mt-2 leading-relaxed opacity-80">Barbell, mesin, & dumbbell.</p>
                            </div>
                        </button>

                        <button @click="selectMode('calist')" class="p-8 text-left hover:bg-[#fcfbf7] transition-colors group flex flex-col gap-4">
                            <Activity class="w-10 h-10 text-primary group-hover:scale-110 transition-transform" :stroke-width="1.5" />
                            <div>
                                <h4 class="text-2xl font-black uppercase text-foreground-primary group-hover:text-primary transition-colors">CALIST</h4>
                                <p class="font-mono text-xs text-foreground-text mt-2 leading-relaxed opacity-80">Pull-up bar, parallettes, & band.</p>
                            </div>
                        </button>
                    </div>
                </div>
        ...
        <transition name="fade">
            <div v-if="showLogoutModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                <div class="w-full max-w-sm bg-white border-2 border-foreground-primary p-8 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] text-center relative animate-bounce-in">
                    ...
                    <p class="font-mono text-sm mb-8 text-foreground-text leading-relaxed">Your session will be ended.</p>
                    <div class="flex flex-col gap-3">
                        <button @click="confirmLogout" class="w-full py-4 bg-foreground-primary text-white font-bold text-lg uppercase hover:bg-red-600 hover:border-red-600 transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)]">Yes, I'm Done</button>
                        <button @click="showLogoutModal = false" class="w-full py-4 bg-transparent border-2 border-separator text-foreground-text font-bold text-lg uppercase hover:border-foreground-primary hover:text-foreground-primary transition-colors">Cancel</button>
                    </div>
                </div>
            </div>
        </transition>
...
<style scoped>
.menu-enter-active, .menu-leave-active { transition: all 0.3s ease-in-out; }

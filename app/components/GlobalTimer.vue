<template>
    <div
        v-if="isVisible"
        class="fixed bottom-6 left-1/2 -translate-x-1/2 text-white px-4 py-3 rounded-full shadow-2xl flex items-center gap-3 md:gap-4 z-[9999] border-2 transition-all duration-300"
        :class="[
            isRinging
    ...
    >
        <div class="flex items-center gap-2">
            ...
            <div
                class="font-mono text-xl md:text-2xl font-bold w-16 md:w-20 text-center tracking-wider"
                :class="{
                    'text-red-400':
                        isRinging || (timerActive && timerSeconds <= 10),
                }"
            >
                {{ formattedTime }}
            </div>
        ...
        <template v-else>
            <div
                class="flex gap-1 md:gap-2 border-l border-slate-600 pl-3 md:pl-4"
            >
                <button
                    type="button"
                    @click="addTime(30)"
            ...
            >
                <button
                    ...
                    :class="
                        timerActive ? 'text-yellow-400' : 'text-emerald-400'
                    "
                >
                    <Pause
                        v-if="timerActive"
                    ...
                    <Play
                        v-else
                        class="w-4 h-4 md:w-5 md:h-5 fill-current ml-0.5"
                    />
                </button>
                <button
                    type="button"

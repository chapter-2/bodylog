...
const draftKey = computed(() => `bodylog_draft_gym_${props.week}_${props.day}`);
    ...
    const sets = Array(ex.sets)
      ...
      .map((_, i) => {
        if (existing && existing.sets[i]) {
          return {
            weight: existing.sets[i].weight,
            reps: existing.sets[i].reps,
          };
        }
...
onMounted(async () => {
  ...
  try {
    const res = (await secureFetch("/api/program/get?mode=gym").catch(
      () => ({}),
    )) as any;
